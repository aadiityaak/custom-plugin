...
jQuery(document).ready(function($) {
    ...
    function addCustomQuickEditFields($editRow, $row) {
        ...
        if ($row.find('.column-product_price').length) {
            var currentPrice = $row.find('.column-product_price strong').text().replace(/[^0-9.,]/g, '');
            
            var priceField = $('<label class="inline-edit-group">' +
                '<span class="title">Harga</span>' +
                '<input type="number" name="custom_product_harga" value="' + currentPrice + '" step="0.01" min="0">' +
                '</label>');
            
            $editRow.find('.inline-edit-col-right .inline-edit-col').append(priceField);
        }
        ...
        if ($row.find('.column-order_status').length) {
            ...
            currentStatus = currentStatus ? currentStatus.replace('status-', '') : 'pending';
            ...
            $.each(statusOptions, function(i, option) {
                ...
                statusSelect.append('<option value="' + option.value + '"' + selected + '>' + option.label + '</option>');
            });
            
            var statusField = $('<label class="inline-edit-group">' +
                '<span class="title">Status</span>' +
                '</label>').append(statusSelect);
            
            $editRow.find('.inline-edit-col-right .inline-edit-col').append(statusField);
        }
    }
    ...
    function addBulkStatusActions() {
        if ($('.wp-list-table').data('post-type') === 'custom_order') {
            ...
            $.each(statusOptions, function(i, option) {
                $bulkSelect.append('<option value="' + option.value + '">' + option.text + '</option>');
                $bulkSelect2.append('<option value="' + option.value + '">' + option.text + '</option>');
            });
        }
    }
    ...
    function makeTableResponsive() {
        $(window).on('resize', function() {
            ...
            if (windowWidth < 1200) {
                $('.wp-list-table .column-featured_image, .wp-list-table .column-product_price').hide();
            } else {
                $('.wp-list-table .column-featured_image, .wp-list-table .column-product_price').show();
            }
